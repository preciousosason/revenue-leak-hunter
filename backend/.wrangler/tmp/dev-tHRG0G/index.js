...
function generatePortalToken() {
  ...
  for (let group = 0; group < 4; group++) {
    let value = "";
    for (let i = 0; i < 4; i++) {
      const index = randomValues[group * 4 + i] % characters.length;
...
__name(generatePortalToken, "generatePortalToken");
...
function validateContact(data) {
  ...
  if (data.message && (typeof data.message !== "string" || data.message.length > 5e3)) {
...
async function handlePortalLogin(request, env) {
  ...
  if (!data.token || typeof data.token !== "string") {
    return json(
      {
        success: false,
        error: "Please enter your private access token."
      },
      400
    );
  }
  ...
  if (!/^LH-[A-Z0-9]{4}(?:-[A-Z0-9]{4}){3}$/.test(token)) {
    return json(
      {
        success: false,
        error: "Invalid private access token."
      },
      400
    );
  }
...
async function handlePortalMe(request, env) {
  ...
  try {
    ...
    return json(
      {
        success: false,
        error: "Unable to load your portal."
      },
      500
    );
  }
...
var src_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" && request.method === "GET") {
      return json({
        success: true,
        service: "Revenue Leak Hunter API",
        status: "online"
      });
    }
...
// .wrangler/tmp/bundle-EMMgAK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-EMMgAK/middleware-loader.entry.ts
